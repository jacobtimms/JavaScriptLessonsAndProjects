...
//Ammended hasSome function -
//The return was previously closing the function after testing the rules against only the first object in the array.
let hasSome = (arr, someRules) => {
    for (var i=0; i<=arr.length-1; i++) {
        if (someRules(arr[i]) === true) {
            return true;
        }
    }return false;
}